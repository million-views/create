...
import { ProjectDetector } from './analyzers/project-detector.mjs';
import { PlaceholderFinder } from './analyzers/placeholder-finder.mjs';
import { FileScanner } from './analyzers/file-scanner.mjs';
import FileProcessor from './processors/file-processor.mjs';
import { CleanupProcessor } from './processors/cleanup-processor.mjs';
import { SetupGenerator } from './generators/setup-generator.mjs';
import { MetadataGenerator } from './generators/metadata-generator.mjs';
import { UndoLogManager } from './restoration/undo-log-manager.mjs';
import { Sanitizer } from './restoration/sanitizer.mjs';
import { Logger } from '../../../lib/shared/utils/logger.mjs';
import { exists, writeFileAtomic } from '../../../lib/shared/utils/fs-utils.mjs';
// import { SchemaValidator } from './utils/schema-validator.mjs';
import { ERROR_CODES } from './config.mjs';
import { MakeTemplateError } from './utils/errors.mjs';
import { readFileAsText, detectTopLevelSideEffects, hasSetupExport } from './utils/fixture-safety.mjs';
...
export class ConversionEngine {
  ...
  async analyzeProject(options) {
    ...
    if (projectType === 'generic') {
      const hasWrangler = await exists('wrangler.jsonc');
      const hasVite = await exists('vite.config.js') || await exists('vite.config.ts');
    ...
    try {
      ...
      let readmeContent = '';
      if (readmeExists) {
        try { readmeContent = await readFile('README.md', 'utf8'); } catch (_e) { readmeContent = ''; }
      }
          ...
          switch (ph.name) {
            ...
            default:
              ...
              if (ph.files && ph.files.includes('wrangler.jsonc')) {
                try {
                  const wranglerExists = await exists('wrangler.jsonc');
                  if (wranglerExists) {
                    const wranglerContent = await readFile('wrangler.jsonc', 'utf8');
                    const clean = wranglerContent.replace(/\/\*[\s\S]*?\*\//g, '');
                    const wranglerConfig = JSON.parse(clean);
                    ...
                    if (wranglerConfig.d1_databases &&
                        ...
                        wranglerConfig.d1_databases.length > 0) {
      ...
      try {
        if (projectType === 'vite-react') {
          if (await exists('vite.config.js')) {
  ...
  async validateProjectConfiguration(projectType, options = {}) {
    ...
    if (projectType === 'vite-react') {
      const viteConfigExists = await exists('vite.config.js') || await exists('vite.config.ts');
      ...
      for (const action of modifyActions) {
        ...
        try {
          ...
          if (disk && disk.ok) {
            ...
            if (action.file === 'README.md' && humanTitle && originalForDisplay.includes(humanTitle)) {
              ...
              if (readmePh && readmePh.placeholder) {
                ...
                if (!readmeValueLooksTemplated) {
                  if (!reverseReplacements.find(r =>
                    r.token === readmePh.placeholder && r.val === humanTitle
                  )) {
                    reverseReplacements.push({
                      token: readmePh.placeholder,
                      val: String(humanTitle),
                      isFileScoped: true
                    });
                  }
                } else {
                ...
                if (projPh && projPh.placeholder) {
                  if (!reverseReplacements.find(r =>
                    r.token === projPh.placeholder && r.val === humanTitle
                  )) {
                    reverseReplacements.push({
                      token: projPh.placeholder,
                      val: String(humanTitle),
                      isFileScoped: !!(projPh.files && projPh.files.includes(action.file))
                    });
                  }
                }
              }
            ...
            const origLines = shouldCollapse ? [originalForDisplay.replace(/\s*\n\s*/g, ' ')] : originalForDisplay.split('\n');
            const repLines = shouldCollapse ? [replacedPreview.replace(/\s*\n\s*/g, ' ')] : replacedPreview.split('\n');
  ...
  addTemplateMetadataDescription(projectType) {
    ...
    this.logger.info('     - v1.0.0 schema with setup, featureSpecs, and constants sections');
    this.logger.info('     - "setup" section with dimensions, gates, and policy');
    ...
    switch (projectType) {
      case 'cf-d1':
        this.logger.info('     - Cloudflare Worker D1 specific features (database, auth, cors)');
        this.logger.info('     - wrangler.jsonc configuration');
        break;
      case 'cf-turso':
        this.logger.info('     - Cloudflare Worker Turso specific features (database, turso)');
        break;
      case 'vite-react':
        this.logger.info('     - React application features (typescript, testing)');
        break;
      default:
        this.logger.info('     - Node.js application features (testing, docs)');
        break;
    }
  }
  ...
  async executePlan(plan) {
    ...
    try {
      ...
      if (createActions.length > 0) {
        ...
        for (const action of createActions) {
          await writeFileAtomic(action.file, action.content);
