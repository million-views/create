...
function validateV1TemplateManifest(manifest) {
  ...
  if (!manifest.placeholderFormat || typeof manifest.placeholderFormat !== 'string') {
    throw new ValidationError('V1 template must specify placeholderFormat (unicode, mustache, dollar, or percent)', 'placeholderFormat');
  }
  ...
  if (!validFormats.includes(manifest.placeholderFormat)) {
    throw new ValidationError(`Invalid placeholderFormat: ${manifest.placeholderFormat}. Must be one of: ${validFormats.join(', ')}`, 'placeholderFormat');
  }
    ...
    for (const [placeholderName, config] of Object.entries(manifest.placeholders)) {
      ...
      if (config.default !== undefined) {
        ...
        if (defaultType !== 'string' && defaultType !== 'number' && defaultType !== 'boolean') {
          throw new ValidationError(`placeholder '${placeholderName}' default must be string, number, or boolean`, `placeholders.${placeholderName}.default`);
        }
      }
      ...
      normalizedPlaceholders.push({
        ...
        default: config.default, // Can be undefined (optional per new schema)
        required: config.required !== undefined ? config.required : true, // New schema: defaults to true
        sensitive: config.secure || false, // New schema uses 'secure' instead of 'sensitive'
        type: config.type || 'text' // New schema: defaults to 'text' (was 'string' in old schema)
      });
    }
  }
