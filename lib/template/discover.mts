...
export class TemplateDiscovery {
  ...
  /**
   * List all templates in a repository or local directory
   * @param {string} repoUrl - Repository URL, user/repo format, or local path
   * @param {string} branchName - Git branch name (ignored for local paths)
   * @returns {Array} - Array of template objects with metadata
   */
  async listTemplates(repoUrl, branchName = 'main') {
    // Check if this is a local path (doesn't look like a URL)
    const isLocalPath = !repoUrl.includes('://') && !repoUrl.includes('@');

    if (isLocalPath) {
      // Local directory - list templates directly
      return await this.listTemplatesFromPath(repoUrl);
    }

    // Remote repository - use cache
  ...
  async listTemplatesFromPath(repoPath) {
    ...
    const errors = await File.validateDirectoryExists(repoPath, 'Repository path');
    if (errors.length > 0) {
  ...
  async parseTemplateJson(templatePath) {
    ...
    try {
      return await File.readJsonFile(templateJsonPath, null, 'template.json');
    } catch {
      return null;
    }
  }
  ...
  async parsePackageJson(templatePath) {
    ...
    try {
      return await File.readJsonFile(packageJsonPath, null, 'package.json');
    } catch {
      return null;
    }
  }
  ...
  formatTemplateOptions(templates) {
    return templates.map((template, index) => ({
      ...
      handoff: Array.isArray(template.handoff) ? template.handoff : [],
...
export async function loadTemplateMetadataFromPath(templatePath) {
  ...
  try {
    const rawContent = readFileSync(templateJsonPath, 'utf8');
    ...
    if (data.schemaVersion === '1.0.0') {
      ...
      if (data.placeholders) {
        if (typeof data.placeholders === 'object' && !Array.isArray(data.placeholders)) {
          // Convert object format to array format expected by normalizePlaceholders
          for (const [placeholderName, config] of Object.entries(data.placeholders)) {
      ...
      const finalPlaceholders = normalizePlaceholders
        ? normalizePlaceholders(normalizedPlaceholders)
      ...
      return {
        ...
        canonicalVariables: data.canonicalVariables || [],
    ...
    if (error instanceof SyntaxError) {
      throw new Error(
        `template.json contains invalid JSON: ${sanitize.error(error.message)}`
      );
    }
    ...
    throw new Error(
      `Failed to load template metadata: ${sanitize.error(error.message)}`
    );
  }
}
