...
├── template.json            # Template metadata and configuration (required)
...
**Special Files:**
- **`template.json`**: Template metadata and configuration file (required). Defines template schema version, placeholders, dimensions, features, and other template characteristics. Generated by `create template` commands and validated by `create scaffold`.
