...
export function validateTemplateManifest(manifest) {
  ...
  // TEMPORARY: Allow legacy templates for test fixtures during pre-1.0 development
  // TODO: Remove this fallback when all templates are migrated to V1.0.0
  console.warn(`⚠️  DEPRECATED: Template without schemaVersion detected. Please migrate to schemaVersion: "1.0.0" format. Legacy support will be removed at v1.0.0.`);
 ...
 * @deprecated Will be removed at v1.0.0 - migrate to V1.0.0 schema
 */
function validateLegacyTemplateManifest(manifest) {
  ...
  const handoff = normalizeHandoff(manifest.handoff);
  ...
  const authoring = validateAuthoringMode(setup?.authoring);
