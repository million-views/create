...
export function detectTopLevelSideEffects(sourceText) {
  ...
  for (const p of patterns) {
    ...
    for (const line of lines) {
      if (p.rx.test(line)) {
        ...
        if (/placeholder mapping/i.test(line) ||
            /PLACEHOLDER_MAP/.test(line) ||
            /tools\.placeholders/.test(line)) continue;
        warnings.push(p.msg);
        break;
      }
    }
  ...
  if (warnings.length > 0) {
    ...
    try { console.warn('fixture-safety: matched warnings ->', warnings); } catch (_e) { }
    return warnings;
  }
