...
import { createTemplateIgnoreSet, shouldIgnoreTemplateEntry } from '@m5nv/create/lib/template/ignore.mts';
...
import { ErrorContext, ErrorSeverity } from '@m5nv/create/lib/error/handler.mts';
...
export class GuidedSetupWorkflow {
  ...
  async #validateLoadedSelections(selections) {
    try {
      ...
      const { SelectionValidator } = await import('../../../../../lib/validation/selection-validator.mts');
  ...
  async #displayWorkflowCompletion() {
    ...
    await this.prompt.write('═'.repeat(50) + '\n');
    await this.prompt.write('Created with @m5nv/create\n\n');
  ...
  async #applyPlaceholdersToFile(filePath, placeholders) {
    try {
      ...
      const { createTokenPattern } = await import('../../../../../lib/placeholder/format.mts');
