...
export class RestorationProcessor {
  ...
  async createBackups(conflicts, options = {}) {
    ...
    for (const conflict of conflicts) {
      try {
        ...
        // Create backup
        await copyFile(conflict.path, finalBackupPath);
  ...
  async resolveConflicts(conflicts, options = {}) {
    const { strategy: _strategy = 'prompt', backup = true, force = false } = options;
  ...
  async restoreFile(action) {
    try {
      ...
      if (content !== null && content !== undefined) {
        ...
        if (action.applyPlaceholderReplacementsOnRestore &&
  ...
  async rollbackRestoration(result, backups) {
    ...
    if (backups && backups.backups) {
      for (const backup of backups.backups) {
        try {
          await copyFile(backup.backup, backup.original);
          await remove(backup.backup);
