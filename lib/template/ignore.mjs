 ...
 *
 * IMPORTANT: Template asset directories (templates/, __scaffold__/, etc.) and
 * _setup.mjs are intentionally NOT in this list because:
 * 1. They must be copied to project directory during scaffolding
 * 2. _setup.mjs executes and reads assets from the project directory (not cache)
 * 3. Workflow cleanup removes them after setup completes
 */
const BASE_TEMPLATE_ARTIFACTS = Object.freeze([
  ...
  '.templatize.json',
  'node_modules',
  'template.json'       // Template metadata (V1.0.0 schema) - never needed in project
]);
