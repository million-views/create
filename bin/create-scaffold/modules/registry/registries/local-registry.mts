...
import { BaseRegistry } from './base-registry.mts';
...
export class LocalRegistry extends BaseRegistry {
  constructor(options = {}, dependencies = {}) {
    ...
    this.TemplateDiscovery = dependencies.TemplateDiscovery || TemplateDiscovery;
    this.loadTemplateMetadataFromPath = dependencies.loadTemplateMetadataFromPath || loadTemplateMetadataFromPath;
    ...
    try {
      ...
      if (this.options.templates) {
        ...
        for (const [templateName, templatePath] of Object.entries(this.options.templates)) {
          try {
            const templateInfo = await this.loadTemplateMetadataFromPath(templatePath);
            templates.push({
              id: templateName,
              name: templateName,
              path: templatePath,
        ...
        const discovery = new this.TemplateDiscovery(null); // No cache manager needed for local
