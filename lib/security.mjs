...
// Import and re-export ValidationError from the error domain for backward compatibility
...
// ValidationError is now re-exported from ./error/index.mts at the top of this file
...
export function validateAuthoringMode(mode) {
  ...
  if (!allowed.includes(normalized)) {
    throw new ValidationError(
      `setup.authoring must be one of: ${allowed.join(', ')}`,
      'authoring'
    );
  }
...
export function validateAllInputs(inputs) {
  ...
  try {
    if (inputs.projectName) {
      validated.projectName = validateProjectDirectory(inputs.projectName);
    }
  } catch (error) {
  ...
  const safeFlags = ['cache', 'dryRun', 'inputPrompts', 'config', 'placeholders', 'selection'];
  for (const flag of safeFlags) {
    if (inputs[flag] !== undefined) {
      validated[flag] = inputs[flag];
    }
  }
