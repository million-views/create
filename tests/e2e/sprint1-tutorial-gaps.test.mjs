 ...
 *
 * NOTE: The current implementation uses 'selections' and 'version' keys in selection files,
 * which differs from the schema that specifies 'choices' and 'schemaVersion'. The tests
 * use the format expected by the current implementation.
 *
 * Additionally, the current implementation only loads dimension selections from selection files,
...
test('Selection file via CLI - scaffolds with dimension selections from file', async (t) => {
  ...
  // Create selection file with dimension selections
  // Note: Current implementation uses 'selections' and 'version' keys
...
test('Gates validation - valid dimension combination succeeds', async (t) => {
  ...
  // Note: Using 'selections' and 'version' as expected by current implementation
