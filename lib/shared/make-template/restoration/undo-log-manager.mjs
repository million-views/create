...
import { readFile, writeFile as _writeFile, access, stat } from 'node:fs/promises';
import { constants } from 'node:fs';
import { join as _join, basename as _basename, dirname as _dirname } from 'node:path';
import { FileCategorizer } from '../utils/file-categorizer.mjs';
import { writeFileAtomic } from '../../../../lib/shared/utils/fs-utils.mjs';
...
export class UndoLogManager {
   ...
   * @param {Object} options - Options
   */
  async processModifiedFile(undoLog, action, _options = {}) {
    try {
   ...
   * @param {Object} options - Options
   */
  async processDeletedItem(undoLog, action, _options = {}) {
    try {
   ...
   * @param {Object} options - Options
   */
  async processCreatedFile(undoLog, action, _options = {}) {
    try {
   ...
   */
  async sanitizeUndoLog(undoLog, _options = {}) {
    try {
