...
import { ArgumentError } from '../shared/utils/error-classes.mjs';
...
export const GLOBAL_OPTIONS = {
  ...
  'help-advanced': {
    type: 'boolean',
    description: 'Show advanced help with detailed information'
  },
  verbose: {
    type: 'boolean',
...
export function parseArguments(argv = process.argv.slice(2), commandDefinitions = {}) {
  ...
  // Check for global options that might appear after the command (like --help)
  const afterCommandFiltered = [];
  const additionalGlobalOptions = {};

  for (const arg of afterCommand) {
    if (arg === '--help' || arg === '-h') {
      additionalGlobalOptions.help = true;
    } else if (arg === '--version' || arg === '-v') {
      additionalGlobalOptions.version = true;
    } else if (arg === '--verbose') {
      additionalGlobalOptions.verbose = true;
    } else if (arg === '--json') {
      additionalGlobalOptions.json = true;
    } else if (arg.startsWith('--log-file=')) {
      additionalGlobalOptions['log-file'] = arg.split('=', 2)[1];
    } else if (arg === '--help-advanced') {
      additionalGlobalOptions['help-advanced'] = true;
    } else {
      afterCommandFiltered.push(arg);
    }
  }

  // Merge additional global options
  const mergedGlobalOptions = { ...globalParse.values, ...additionalGlobalOptions };

  // Parse command-specific options and positionals
  const commandParse = parseArgs({
    args: afterCommandFiltered,
...
export function validateArguments(parsed, commandDefinitions = {}) {
  ...
  // If help is requested, skip command validation
  if (parsed.globalOptions.help || parsed.globalOptions['help-advanced']) {
    return;
  }

  // If no command specified, help or version should be shown
  if (!parsed.command && !parsed.globalOptions.version) {
