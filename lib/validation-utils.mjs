 ...
 */
export function handleValidationError(validationFn, value, errorArray, fallbackMessage) {
  try {
 ...
 */
export function validateMultipleFields(validations) {
  const errors = [];
  const validated = {};

  for (const { key, fn, value, fallback } of validations) {
    const result = handleValidationError(fn, value, errors, fallback);
    if (result !== null) {
