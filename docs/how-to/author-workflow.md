...
This guide helps template authors iterate efficiently while respecting the division of responsibilities between create template (author tooling) and @m5nv/create (consumer tooling). Follow these loops to keep your templates reliable and predictable.

> **Remember:** Whenever you change files that still contain `{{TOKEN}}` markers, keep `metadata.placeholders` in `template.json` in sync so tooling (including `create template`) knows what inputs to collect from users. Add `metadata.variables` entries when you want the CLI to supply canonical tokens such as `{{AUTHOR}}` or `{{LICENSE}}` with standardized prompts.
...
1. **Restore the working app**
   ...
   ```
   > The command above is generated by create template and rehydrates the project based on `.template-undo.json`.
...
5. **Verify combinations**
   ...
   # Specific feature mix (use selection.json for complex configurations)
...
# Or restore from template if using create template
create template restore
```
...
- If you vendor @m5nv/create in the repo, run `npm run schema:check` to confirm `template.json` still matches the published schema and generated types.
...
Following these loops keeps template iteration fast while ensuring @m5nv/create users receive clean, deterministic scaffolds every time.
