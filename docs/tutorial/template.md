...
description:
  "Learn to create templates using create template, building a complete web presence for a lawn care service—from basic React SPA to marketing website and digital transformation app"
type: tutorial
...
- **Manual vs Automated**: You could edit files by hand, but `create template` automates the detection and replacement
...
create template config validate .templatize.json
```

> **Note**: Configuration validation is automatically performed when you run `create template convert`, so this step is optional. Use `config validate` when you want to verify your configuration without running the full conversion process.
...
**The `create template init` command creates both configuration files:**
...
- **Template testing**: Use `create template test` to validate complex templates before distribution
...
- **Init automation**: `create template init` generates configuration files
- **Manual vs automated**: Understanding when to use auto-detection vs custom rules
- **Template testing**: Use `create template test` to validate templates before distribution
...
- **[Create Scaffold Tutorial](scaffold.md)** - Use these templates to
  scaffold new projects
- [Template Validation](../reference/cli-reference.md#create template-commands) -
...
(run `create template init` if missing)
